#!/usr/bin/env python3
"""Send an optional production monitor failure notification.
...
def build_message(report: dict[str, Any]) -> str:
    ...
    deployment_id = next(
        (
            ...
            for check in checks
            if str(check.get("name") or "").endswith("_version_endpoint")
            and check.get("deployment_id")
        ),
        "",
    )

    lines = [
        "KMFX Production Monitor fallo",
...
def post_webhook(url: str, message: str) -> tuple[bool, str]:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "KMFX-Production-Monitor-Notifier/1.0",
        },
...
def main() -> int:
    ...
    parser.add_argument("--report", default="production-monitor.json")
    args = parser.parse_args()

    webhook_url = (
        os.environ.get("PRODUCTION_MONITOR_WEBHOOK_URL")
        or os.environ.get("BETA_MONITOR_WEBHOOK_URL")
        or ""
    ).strip()
    if not webhook_url:
        print("PRODUCTION_MONITOR_WEBHOOK_URL not set; relying on GitHub Actions failure notification.")
        return 0
    ...
    if ok:
        print(f"production monitor notification sent: {detail}")
        return 0

    print(f"production monitor notification failed: {detail}", file=sys.stderr)
    return 1
