...
class UserFlowUiContractTests(unittest.TestCase):
    ...
    def test_mt5_account_key_flow_copy_keeps_stable_key_language(self) -> None:
        ...
        self.assertIn("Nombre visible de la cuenta", wizard)
        self.assertIn("Tipo de cuenta", wizard)
        self.assertIn('select name="eaAccountProfile"', wizard)
        self.assertIn('select name="directAccountProfile"', wizard)
        self.assertIn("Cuenta Funding MT5", wizard)
        self.assertIn("Cuenta Challenge MT5", wizard)
    ...
    def test_critical_auth_admin_and_billing_modules_share_fresh_cache_key(self) -> None:
        ...
        for relative_path in files:
            ...
            for module in ("auth-session", "admin-mode", "billing-status", "connection-wizard", "connections", "download-artifacts"):
                ...
                if marker in source:
                    self.assertIn(
                        f"{module}.js?v=build-20260514-222200",
                        source,
    ...
    def test_dashboard_shell_uses_current_production_cache_key(self) -> None:
        ...
        self.assertIn('src="./app.js?v=build-20260514-222200"', html)
        self.assertIn('const BUILD_TAG = "build-20260514-222200";', app)
        for module in ("dashboard", "connections", "sidebar-ui", "auth-ui"):
            self.assertIn(
                f"./js/modules/{module}.js?v=build-20260514-222200",
                app,
