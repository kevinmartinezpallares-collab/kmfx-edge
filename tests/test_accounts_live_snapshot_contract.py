...
class AccountsLiveSnapshotContractTests(unittest.TestCase):
    def test_live_snapshot_polling_backs_off_to_reduce_supabase_egress(self) -> None:
        ...
        self.assertIn("PRODUCTION_FULL_SNAPSHOT_REFRESH_MS_ACTIVE = 24 * 60 * 60 * 1000", source)
        self.assertIn("PRODUCTION_FULL_SNAPSHOT_REFRESH_MS_IDLE = 24 * 60 * 60 * 1000", source)
        self.assertIn("PRODUCTION_FULL_SNAPSHOT_REFRESH_MS_HIDDEN = 24 * 60 * 60 * 1000", source)
        ...
        self.assertIn("return hasOpenPositions ? 20000 : 180000", source)
        ...
        self.assertIn("if (!isLocalRuntime()) return false;", source)
        ...
        self.assertIn('KMFX_BANDWIDTH_EMERGENCY_LOCKDOWN", default=_is_production_runtime()', backend)
        self.assertIn('return _env_int("KMFX_BANDWIDTH_HARD_SYNC_INTERVAL_SECONDS", default=300)', backend)
