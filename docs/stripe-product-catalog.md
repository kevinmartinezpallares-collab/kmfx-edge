...
Note: the Stripe connector created Prices without lookup keys or metadata. Until those fields are completed in Stripe Dashboard/API, configure the backend with the live Price IDs from `docs/billing-env-vars.md`. As of 2026-05-10, Render production has the six live Price IDs configured, so checkout does not depend on lookup keys.
...
On 2026-05-10, Stripe MCP confirmed the six live Prices and the coupon `KMFX Comunidad Privada` with `percent_off=100`, duration `forever`. Render API confirmed the backend uses the six live Price IDs directly. Lookup keys still need to be added in Stripe Dashboard/API before replacing the Price IDs with lookup keys.
