...
- `npx react-doctor@latest --verbose --diff`: `100 / 100`, `8 issues`
...
- `settings/reference-sections`: aplicado split de `SettingsReferenceSection`; ese componente ya no aparece como `no-giant-component` en el diff de React Doctor. Queda pendiente `SubscriptionReferenceSection`.
