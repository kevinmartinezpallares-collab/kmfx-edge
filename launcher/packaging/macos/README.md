...
That ZIP is created from the verified distribution copy of the app bundle:

```bash
scripts/build_macos_launcher.sh --zip
```
...
The build script clears extended attributes, applies an ad-hoc signature for local testing, verifies the signed temporary copy, and creates the public ZIP from that clean copy. Use a Developer ID certificate plus notarization before removing the normal Gatekeeper warning from public distribution.
