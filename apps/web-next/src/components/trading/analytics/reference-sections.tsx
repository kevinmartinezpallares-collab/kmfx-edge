        ...
        <div className="grid gap-4 xl:grid-cols-2 xl:items-start">
          <div className="grid gap-4">
            <Card className="border-border/70 bg-card/70">
              ...
              <CardContent className="grid gap-4">
                <SessionSignalMap rows={insights.sessionRows} />
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {[
                    ...
                    },
                    {
                      ...
                      note: worstHour ? `${formatSignedCurrency(worstHour.pnl)} / ${formatOperationCount(worstHour.trades)}` : "Sin hora negativa",
                    ...
                    <div key={item.label} className="min-w-0">
                      ...
                      <p className={cn("mt-1 font-mono text-xs font-semibold", insightFindingClasses(item.tone as "positive" | "negative" | "warning" | "neutral"))}>
                        {item.note}
                      </p>
                    </div>
                  ))}
          ...
          <div className="grid gap-4">
            <Card className="border-border/70 bg-card/70">
              ...
              <CardContent>
                <AttributionSignalMap rows={symbolRows.slice(0, 5)} />
              </CardContent>
            </Card>
              ...
              <CardContent className="grid gap-5 md:grid-cols-[160px_1fr] xl:grid-cols-1">
                ...
                <div className="grid gap-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ["PF", performance.profitFactor.toFixed(2), "neutral"],
                      ["Expectativa", formatSignedCurrency(performance.expectancy), performance.expectancy >= 0 ? "positive" : "negative"],
                      ["Operaciones", `${performance.totalTrades}`, "neutral"],
                    ].map(([label, value, tone]) => (
                      <div key={label} className="min-w-0">
                        <p className="text-xs text-muted-foreground">{label}</p>
                        <p className={cn("mt-2 font-mono text-sm font-semibold", insightFindingClasses(tone as "positive" | "negative" | "warning" | "neutral"))}>
                          {value}
                        </p>
                      </div>
                    ))}
                ...
                <div key={label} className="min-w-0">
                  ...
                  <p
                    className={cn(
                      ...
                      insightFindingClasses(tone as "positive" | "negative" | "warning" | "neutral"),
                    )}
                  >
                    {value}
                  </p>
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      variant="outline"
                      size="icon"
                      disabled={safeSelectedMonthIndex <= 0}
                    ...
                    <div className="min-w-40 rounded-lg border border-border/70 bg-background/35 px-3 py-2 text-center">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {monthActiveDays.length} días operados
                      </p>
                    </div>
                    <Button
                      type="button"
                      variant="outline"
                      size="icon"
                      disabled={safeSelectedMonthIndex >= monthKeys.length - 1}
                      onClick={() => changeDailyMonth(1)}
                  ...
                  {calendarCells.map((cell) => {
                    ...
                    const tone =
                      !day || day.pnl === 0
                        ...
                        : day.pnl > 0
                          ? "positive"
                          : "negative";
                    const isSelected = selectedDay?.tradingDayKey === cell.key;
                    ...
                    return (
                      <button
                        ...
                        disabled={!day}
                        onClick={() => {
                          if (day) setSelectedDayKey(day.tradingDayKey);
                        }}
                        className={cn(
                        ...
                        title={
                          day
                            ? `${formatOperationCount(day.trades)} / ${formatSignedCurrency(day.pnl)}`
                            : "Sin operativa"
                        }
                        aria-label={
                          day
                ...
                <div className="grid divide-y divide-border/60 border-t border-border/60 sm:grid-cols-2 sm:divide-x sm:divide-y-0">
                  <div className="p-4">
                    ...
                    <p className="mt-2 font-mono text-sm font-semibold text-foreground">
                      {positiveMonthDays.length} / {monthActiveDays.length || 0}
                    </p>
                  </div>
                  <div className="p-4">
                    <p className="text-xs text-muted-foreground">Días negativos</p>
                    <p className="mt-2 font-mono text-sm font-semibold text-foreground">
                      {negativeMonthDays.length}
                    </p>
                  </div>
                </div>
              </CardContent>
                ...
                <div className="flex rounded-full border border-border/70 bg-background/35 p-1">
                  {[
                    ...
                    <Button
                      ...
                      onClick={() => setHourValueMode(mode as "currency" | "percent")}
                    >
                      {label}
                    </Button>
                  ))}
            ...
            <CardContent className="grid gap-4">
              <div className="grid gap-5 p-1 sm:p-2">
                <div className="grid gap-2 md:grid-cols-[minmax(0,1fr)_180px] md:items-start">
                  <div className="min-w-0">
                    <p className="text-4xl font-semibold tracking-tight text-foreground">
                      {selectedHeatmapCell
                        ? formatHourValue(selectedHeatmapCell.pnl, hourValueMode)
                        : "Sin datos"}
                    </p>
                    <p className="mt-2 text-sm text-muted-foreground">
                      Selección / {selectedHeatmapCell?.label ?? "sin franja"} /{" "}
                      {formatOperationCount(selectedHeatmapCell?.trades ?? 0)}
                    </p>
                  </div>
                  <div className="pt-1 md:text-right">
                    ...
                    <p className={cn("mt-2 font-mono text-lg font-semibold", insightFindingClasses(totalHourlyPnl >= 0 ? "positive" : "negative"))}>
                      {activeHours.length > 0
                        ? formatHourValue(totalHourlyPnl / activeHours.length, hourValueMode)
                        : "Sin datos"}
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      {formatOperationCount(totalHourlyTrades)} / {totalHourlyWins}W / {totalHourlyLosses}L
                ...
                <div className="grid grid-cols-6 gap-1.5 sm:hidden">
                  {timeline.map((hour) => {
                    ...
                    const isSelected =
                      representative?.key === selectedHeatmapCell?.key ||
                      (!representative && selectedHeatmapCell?.hour === hour.hour);
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          if (representative) setSelectedHeatmapCellKey(representative.key);
                        }}
                        className={cn(
                        ...
                        {(hourlyHeatmapCellsByDay.get(day.key) ?? []).map((cell) => {
                            ...
                            return (
                              <button
                                ...
                                onClick={() => setSelectedHeatmapCellKey(cell.key)}
                                className={cn(
                ...
                <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <p className={cn("font-mono text-xs font-semibold", insightFindingClasses(selectedHeatmapTone))}>
                    {selectedHeatmapCell
                      ? `${selectedHeatmapCell.wins}W / ${selectedHeatmapCell.losses}L`
                      : "Sin selección"}
                  </p>
                </div>
              </div>
              ...
              <div className="pt-2">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Sesión dominante: <span className="font-semibold text-foreground">{hourlyOverview.dominantSession}</span>
                  </p>
                </div>
                <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  {keyHours.map((hour) => {
                    const tone =
                      hour.pnl > 0 ? "positive" : hour.pnl < 0 ? "negative" : "neutral";
