...
export function renderDashboard(root, state) {
  ...
  chartSpecs.push(
    lineAreaSpec("dashboard-hero-equity-chart", heroCurve, {
      ...
      borderWidth: 2.85,
      pointRadius: (context) => (context.dataIndex === heroCurve.length - 1 ? 4.2 : 0),
      pointHoverRadius: (context) => (context.dataIndex === heroCurve.length - 1 ? 4.8 : 3.2),
      pointHitRadius: 20,
      fill: isDarkTheme,
      fillAlphaStart: isDarkTheme ? 0.13 : 0.06,
      fillAlphaEnd: isDarkTheme ? 0.018 : 0.008,
      glowAlpha: 0,
      tension: 0.9,
      animationDuration: 0,
